...
document.querySelector(".submit").addEventListener("click", function () {
  if (!document.querySelector(".ip").value) {
    ...
    document.querySelector("body").style.background =
      "linear-gradient(to left, rgb(0,0,0), rgb(239, 1, 1))";
    if (score > final) {
      final = score;
    ...
    if (score > 1) {
      document.querySelector(".inform").textContent =
        document.querySelector(".ip").value > x
          ? `⬆️you entered bigger number`
          : `⬇️you entered smaller number`;
      score--;
...
document.querySelector(".refresh").addEventListener("click", function () {
  ...
  document.querySelector(".score").textContent = score;
  document.querySelector("body").style.background =
    " linear-gradient(to right, rgb(0,0,0), rgb(239, 1, 1)";
